...
//等待 I2C_S
#define i2c_Wait(I2Cn)              { iic_wait_out = 0;while(iic_wait_out < 300 &&( I2C_S_REG(I2CN[I2Cn]) & I2C_S_IICIF_MASK)==0) {iic_wait_out++;} \
...
uint32 i2c_init(I2Cn_e i2cn, uint32 baud)
{
    ...
    else
    {
        ...
        if(I2C1_SCL_PIN == PTE1)
            port_init (I2C1_SCL_PIN, ALT6 | ODO | PULLUP | HDS);
        else if(I2C1_SCL_PIN == PTC10)
            port_init (I2C1_SCL_PIN, ALT2 | ODO | PULLUP | HDS);
        else
            ASSERT(0);                          //上诉条件都不满足，直接断言失败了，设置管脚有误？
...
uint8 i2c_read_reg(I2Cn_e i2cn, uint8 SlaveID, uint8 reg)
{
   ...
   // i2c_write_byte(i2cn, ( SlaveID << 1 ) | MWSR);      //发送从机地址和写位
    (I2C_D_REG(I2CN[i2cn]) = (( SlaveID << 1 ) | MWSR))
      ;i2c_Wait(i2cn); //0为应答 //0位写位
        
   // i2c_write_byte(i2cn, reg);                          //发送从机里的寄存器地址
    (I2C_D_REG(I2CN[i2cn]) = (reg));
     {
       iic_wait_out = 0;
     while(iic_wait_out < 300 &&( I2C_S_REG(I2CN[i2cn]) & I2C_S_IICIF_MASK)==0) 
     {iic_wait_out++;} 
     I2C_S_REG(I2CN[i2cn]) |= I2C_S_IICIF_MASK;
     }
   ...
   // i2c_write_byte(i2cn, ( SlaveID << 1) | MRSW );      //发送从机地址和读位
    (I2C_D_REG(I2CN[i2cn]) = (( SlaveID << 1 ) | MRSW))
      ;i2c_Wait(i2cn)
...
/*!
 *  @brief      读取I2C设备指定地址寄存器的数据  2个字节
 *  @param      I2Cn_e        I2C模块(I2C0、I2C1)
 *  @param      SlaveID     从机地址(7位地址)
 *  @param      reg         从机寄存器地址
 *  @return                 读取的寄存器值
 *  @since      v5.0
 *  Sample usage:       uint8 value = i2c_read_double_reg(I2C0, 0x1D, 1);
 */

uint16 i2c_read_double_reg(I2Cn_e i2cn,uint8 SlaveID, uint8 reg)
{
    uint8 pBuffer[2];
   ...
   pBuffer[0] = I2C_D_REG(I2CN[i2cn]);                     //虚假读取一次，启动接收数据
    i2c_Wait(i2cn);                                     //等待接收完成
    

    i2c_DisableAck(i2cn);                                //禁用应答(接收最后一个字节)
    
    pBuffer[0] = I2C_D_REG(I2CN[i2cn]);                     //读取数据
    i2c_Wait(i2cn);
    
    i2c_Stop(i2cn);                                     //发送停止信号
    
    // Read third byte
    pBuffer[1] = I2C_D_REG(I2CN[i2cn]);                     //读取数据
    i2c_delay();
