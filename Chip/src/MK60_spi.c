...
uint32 spi_init(SPIn_e spin, SPI_PCSn_e pcs, SPI_CFG master,uint32 baud)
{
    ...
    if(spin == SPI0)
    {
        ...
        if(pcs & SPI_PCS1)
            gpio_init(SPI0_PCS1_PIN,GPO,1);
        //port_init(SPI0_PCS1_PIN, ALT2  );

        if(pcs & SPI_PCS2)
            gpio_init(SPI0_PCS2_PIN ,GPO,1);
        //port_init(SPI0_PCS2_PIN, ALT2  );

        if(pcs & SPI_PCS3)
            gpio_init(SPI0_PCS3_PIN ,GPO,1);
        //port_init(SPI0_PCS3_PIN, ALT2  );

        if(pcs & SPI_PCS4)
            gpio_init(SPI0_PCS4_PIN ,GPO,1);
        //port_init(SPI0_PCS4_PIN, ALT2  );

        if(pcs & SPI_PCS5)
            gpio_init(SPI0_PCS5_PIN ,GPO,1);
    ...
    else if(spin == SPI1)
    {
        ...
        if(pcs & SPI_PCS1)
           gpio_init(SPI1_PCS1_PIN ,GPO,1);
        // port_init(SPI1_PCS1_PIN, ALT2  );

        if(pcs & SPI_PCS2)
           gpio_init(SPI1_PCS2_PIN ,GPO,1);
        // port_init(SPI1_PCS2_PIN, ALT2  );

        if(pcs & SPI_PCS3)
            gpio_init(SPI1_PCS3_PIN ,GPO,1);
    ...
    else if(spin == SPI2)
    {
        ...
        if(pcs & SPI_PCS0)
            gpio_init(SPI2_PCS0_PIN ,GPO,1);
        //port_init(SPI2_PCS0_PIN, ALT2  );

        if(pcs & SPI_PCS1)
            gpio_init(SPI2_PCS1_PIN ,GPO,1);
...
SPI_CSSCK_EXIT:
        ...
        SPI_CTAR_REG(SPIN[spin], 0) = (0
                                       ...
                                       | SPI_CTAR_CPHA_MASK   //数据在SCK第一个边沿改变第二个边沿捕捉。如果是0，则反之。  w25x16在上升沿读取数据；NRF24L01在上升沿读取数据
